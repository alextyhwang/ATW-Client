...
#include "core/agents/javaagents.h"
...
bool AgentsModel::setData(const QModelIndex &index, const QVariant &value, int role) {
    ...
    if (index.column() == Column::OPTION) {
        if (role == Qt::EditRole) {
            Atw::Core::JavaAgents::setOption(agent, value.toString());
            return true;
        }
    }else if(index.column() == Column::NAME){
        if(role == Qt::CheckStateRole){
            Atw::Core::JavaAgents::setEnabled(agent, !agent.enabled);
            return true;
        }
    }
    return false;
...
void AgentsModel::addAgent(const QString &path, const QString &option) {
    ...
    const int row = agents.size();
    beginInsertRows(QModelIndex(), row, row);
    Atw::Core::JavaAgents::add(agents, path, option);
    endInsertRows();
}
...
bool AgentsModel::containsPath(const QString &path) const {
    return Atw::Core::JavaAgents::containsPath(agents, path);
}
